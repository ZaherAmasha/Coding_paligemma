...
class GemmaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # y = torch.gelu(y, approximate="tanh") # [Batch_Size, Seq_Len, Intermediate_Size] # The use gelu is from experimentation, and the tanh approximate refers
        # to an equation that contains a tanh which is easier to compute than the original gelu function
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Repeat the key and value states to match the number of heads of the query
...
class GemmaModel(nn.Module):
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            # [Batch_Size, Seq_Len, Hidden_Size]
            hidden_states = decoder_layer(hidden_states=hidden_states, attention_mask=attention_mask, position_ids=position_ids, kv_cache=kv_cache)
...
class PaliGemmaMultiModalProjector(nn.Module):
    ...
    def forward(self, image_features):
        # [Batch_Size, Num_Patches, Embed_Dim] -> [Batch_Sizee, Num_Patches, Projection_Dim]
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        if kv_cache is None or kv_cache.num_items() == 0:
            ...
            # Also in this case we don't need to mask anything, since each query should be able to attend to all the previous tokens
            # This only works when we have no padding
            causal_mask = torch.full(
